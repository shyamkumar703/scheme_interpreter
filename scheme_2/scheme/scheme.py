...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure1, Pair):
            return Pair(scheme_eval(procedure1, env))
        if scheme_procedurep(procedure1):
            return scheme_apply(procedure1, rest.map(lambda x: scheme_eval(x, env)), env)
        else:
            raise SchemeError('Expression {} is uncallable'.format(procedure1))
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        assignment_one = bindings.first
        first_formal = assignment_one.first
        first_val = scheme_eval(assignment_one.second.first, env)
        formals = Pair(first_formal, formals)
        check_formals(formals)
        vals = Pair(first_val, vals)
        bindings = bindings.second
    return Frame.make_child_frame(env, formals, vals)
...
class Promise:
    ...
    def evaluate(self):
        if self.expression is not None:
            self.value = scheme_eval(
                self.expression, self.env.make_child_frame(nil, nil))
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            if not scheme_listp(result.expr):
                raise SchemeError('malformed list: {0}'.format(repl_str(expr)))
            first, rest = result.expr.first, result.expr.second
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                return SPECIAL_FORMS[first](rest, env)
            else:
                procedure1 = original_scheme_eval(first, env)
                if isinstance(procedure1, Pair):
                    result = Pair(original_scheme_eval(procedure1, env))
                if scheme_procedurep(procedure1):
                    result = scheme_apply(procedure1, rest.map(lambda x: original_scheme_eval(x, result.env)), result.env)
                else:
                    raise SchemeError(
                        'Expression {0} is uncallable'.format(procedure1))
        return result
